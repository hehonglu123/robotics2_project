...
for i=1:N
    ...
    % follow the path
    % ***
%     [val,ind]=min(vecnorm(xc(2:3,i)-despath(1:2,:)));
%     qd(:,i)=[despath(3,ind);despath(1:2,ind)];
%     if ind<length(despath)
%         qdd(:,i)=diff([despath(3,ind:ind+1);despath(1:2,ind:ind+1)]')';    
%     end
    % PD control
