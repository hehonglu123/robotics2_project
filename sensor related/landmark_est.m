...
% stochastic gradient to verify gradient numerically

function J=stoch_grad(x,fun,q)
...
%
function y=h_a(a,q)
    N_range=4;N_bearing=2;
    pL=reshape(a(1:3*N_range),3,N_range);
    pB=reshape(a(1:3*N_bearing),3,N_bearing);    
    pR=[q(1:2);0];
    y_range=vecnorm(pR-pL)';
    y_bearing=(atan2(pB(2,:)-q(2),pB(1,:)-q(1))-q(3))';
    y=[y_range;y_bearing];
